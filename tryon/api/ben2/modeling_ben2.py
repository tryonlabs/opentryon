...
class BEN_Base(
    ...
    def forward(self, x):
        ...
        for i in range(real_batch):
            ...
            input_ = torch.cat(
                (loc_batch, glb_batch[i, :, :, :].unsqueeze(dim=0)), dim=0
            )
    ...
    def inference(self, image, refine_foreground=False):
        ...
        if isinstance(image, Image.Image):
            ...
            if torch.cuda.is_available():
                img_tensor = (
                    img_transform(image).unsqueeze(0).to(next(self.parameters()).device)
                )
            else:
                img_tensor = (
                    img_transform32(image)
                    .unsqueeze(0)
...
def add_audio_to_video(video_without_audio_path, original_video_path, output_path):
    ...
    command = [
        ...
        "0:v:0",
        "-map",
        "1:a:0",  # we know there's an audio track now
        output_path,
    ]
